...
def simulateSpectra(dir_path, element, parent):    
    ...
    def y_calculator(transition_type, fit_type, xfinal, x, y, w, xs, ys, ws, res, energy_values, efficiency_values, enoffset):
        ...
        data.variables.yfinals = [[[0 for n in range(len(xfinal))] for i in label1] for j in range(len(xs))]
        if transition_type == 'Diagram':
            ...
            for j, k in enumerate(y):
                ...
                for i, n in enumerate(k):
                    if fit_type == 'Voigt':
                        data.variables.yfinal[j] = np.add(data.variables.yfinal[j], V(xfinal, x[j][i], y[j][i], res, w[j][i]))
                    elif fit_type == 'Lorentzian':
                        data.variables.yfinal[j] = np.add(data.variables.yfinal[j], L(xfinal, x[j][i], y[j][i], res, w[j][i]))
                    elif fit_type == 'Gaussian':
                        data.variables.yfinal[j] = np.add(data.variables.yfinal[j], G(xfinal, x[j][i], y[j][i], res, w[j][i]))
                    b1 += 100 / (len(y) * len(k))
                    progress_var.set(b1)
            ...
            for j, k in enumerate(ys):
                for l, m in enumerate(ys[j]):
                    ...
                    for i, n in enumerate(m):
                        if fit_type == 'Voigt':
                            data.variables.yfinals[j][l] = np.add(data.variables.yfinals[j][l], V(xfinal, xs[j][l][i], ys[j][l][i], res, ws[j][l][i]))
                        elif fit_type == 'Lorentzian':
                            data.variables.yfinals[j][l] = np.add(data.variables.yfinals[j][l], L(xfinal, xs[j][l][i], ys[j][l][i], res, ws[j][l][i]))
                        elif fit_type == 'Gaussian':
                            data.variables.yfinals[j][l] = np.add(data.variables.yfinals[j][l], G(xfinal, xs[j][l][i], ys[j][l][i], res, ws[j][l][i]))
            ...
            for j, k in enumerate(y):
                ...
                for i, n in enumerate(k):
                    if fit_type == 'Voigt':
                        data.variables.yfinal[j] = np.abs(np.add(data.variables.yfinal[j], V(xfinal, x[j][i], y[j][i], res, w[j][i])))
                    elif fit_type == 'Lorentzian':
                        data.variables.yfinal[j] = np.abs(np.add(data.variables.yfinal[j], L(xfinal, x[j][i], y[j][i], res, w[j][i])))
                    elif fit_type == 'Gaussian':
                        data.variables.yfinal[j] = np.abs(np.add(data.variables.yfinal[j], G(xfinal, x[j][i], y[j][i], res, w[j][i])))
                    b1 += 200 / (len(y) * len(k))
                    progress_var.set(b1)
                    sim.update_idletasks()
                if k != []:  # Excluir as linhas que nao foram seleccionados nos botoes
            ...
            for j, k in enumerate(ys):
                for l, m in enumerate(ys[j]):
                    ...
                    for i, n in enumerate(m):
                        if fit_type == 'Voigt':
                            data.variables.yfinals[j][l] = np.abs(np.add(data.variables.yfinals[j][l], V(xfinal, xs[j][l][i], ys[j][l][i], res, ws[j][l][i])))
                        elif fit_type == 'Lorentzian':
                            data.variables.yfinals[j][l] = np.abs(np.add(data.variables.yfinals[j][l], L(xfinal, xs[j][l][i], ys[j][l][i], res, ws[j][l][i])))
                        elif fit_type == 'Gaussian':
                            data.variables.yfinals[j][l] = np.abs(np.add(data.variables.yfinals[j][l], G(xfinal, xs[j][l][i], ys[j][l][i], res, ws[j][l][i])))
                        b1 += 100 / (len(ys) * len(label1) * len(m))
                        progress_var.set(b1)
                        sim.update_idletasks()
                    if m != []:  # Excluir as linhas que nao foram seleccionados nos botoes
    ...
    def func2min(params, exp_x, exp_y, num_of_points, sat, peak, x, y, w, xs, ys, ws, energy_values, efficiency_values, enoffset):
        ...
        data.variables.ytot, data.variables.yfinal, data.variables.yfinals = y_calculator(sat, peak, xfinal, x, y, w, xs, ys, ws, res, energy_values, efficiency_values, enoffset)
    ...
    def plot_stick(graph_area):
            ...
            if sat != 'Auger':
                ...
                for cs_index, cs in enumerate(charge_states):
                    ...
                    if cs_index < len(rad_PCS):
                        ...
                        mix_val = data.variables.NCS_radMixValues[cs_index - len(rad_PCS)].get()
                        ncs = True
                    if mix_val != '0.0':
                        for transition in the_dictionary:
                            # Se a transição estiver selecionada:
                            if the_dictionary[transition]["selected_state"]:
                                ...
                                if not ncs:
                                    diag_stick_val = [line + [data.variables.PCS_radMixValues[i].get()] for i, linerad in enumerate(lineradrates_PCS) for line in linerad if line[1] in low_level and line[5] == high_level and float(line[8]) != 0 and rad_PCS[i] == cs]  # Cada vez que o for corre, lê o ficheiro de uma transição
                                else:
                                    diag_stick_val = [line + [data.variables.NCS_radMixValues[i].get()] for i, linerad in enumerate(lineradrates_NCS) for line in linerad if line[1] in low_level and line[5] == high_level and float(line[8]) != 0 and rad_NCS[i] == cs]

                                if not ncs:
                                    sat_stick_val = [line + [data.variables.PCS_radMixValues[i].get()] for i, linesat in enumerate(linesatellites_PCS) for line in linesat if low_level in line[1] and high_level in line[5] and float(line[8]) != 0 and sat_PCS[i] == cs]
                                else:
                                    sat_stick_val = [line + [data.variables.NCS_radMixValues[i].get()] for i, linesat in enumerate(linesatellites_NCS) for line in linesat if low_level in line[1] and high_level in line[5] and float(line[8]) != 0 and sat_NCS[i] == cs]
                                    # -------------------------------------------------------------------------------------------
                                if sat == 'Diagram':
                                    if not diag_stick_val:  # Se não ouver dados no vetor da diagrama
                                        # Crio um vector de zeros para o programa continuar a correr (Em string porque estava a dar um erro qq quando punha em int)(range 16 porque é o tamanho da
                ...
                for cs_index, cs in enumerate(charge_states):
                    ...
                    if cs_index < len(aug_PCS):
                        ...
                        mix_val = data.variables.NCS_augMixValues[cs_index - len(aug_PCS)].get()
                        ncs = True
                    if mix_val != '0.0':
                        for transition in the_aug_dictionary:
                            # Se a transição estiver selecionada:
                            if the_aug_dictionary[transition]["selected_state"]:
                                ...
                                if not ncs:
                                    aug_stick_val = [line + [data.variables.PCS_augMixValues[i].get()] for i, lineaug in enumerate(lineaugrates_PCS) for line in lineaug if low_level in line[1] and high_level in line[5][:2] and auger_level in line[5][2:4] and float(line[8]) != 0 and aug_PCS[i] == cs]
                                else:
                                    aug_stick_val = [line + [data.variables.NCS_augMixValues[i].get()] for i, lineaug in enumerate(lineaugrates_NCS) for line in lineaug if low_level in line[1] and high_level in line[5][:2] and auger_level in line[5][2:4] and float(line[8]) != 0 and aug_PCS[i] == cs]
        ...
        # --------------------------------------------------------------------------------------------------------------------------
            ...
            if load != 'No':  # procedimento para fazer o plot experimental
                ...
                exp_spectrum = list(csv.reader(open(load, 'r', encoding='utf-8-sig')))
            ...
            if sat == 'Diagram':
                for index, key in enumerate(the_dictionary):
                    if the_dictionary[key]["selected_state"]:
                        graph_area.plot(xfinal + enoffset, (np.array(data.variables.yfinal[index]) * normalization_var) + y0, label=key, color=str(col2[np.random.randint(0, 7)][0]))  # Plot the simulation of all lines
                        graph_area.legend()
            elif sat == 'Satellites':
                for index, key in enumerate(the_dictionary):
                    if the_dictionary[key]["selected_state"]:
                        for l, m in enumerate(data.variables.yfinals[index]):
                            if max(m) != 0:  # Excluir as linhas que nao foram seleccionados nos botoes
                                graph_area.plot(xfinal + enoffset, (np.array(data.variables.yfinals[index][l]) * normalization_var) + y0, label=key + ' - ' + labeldict[label1[l]], color=str(col2[np.random.randint(0, 7)][0]))  # Plot the simulation of all lines
                                graph_area.legend()
            elif sat == 'Diagram + Satellites':
                for index, key in enumerate(the_dictionary):
                    if the_dictionary[key]["selected_state"]:
                ...
                for index, key in enumerate(the_aug_dictionary):
                    if the_aug_dictionary[key]["selected_state"]:
                        ...
                        graph_area.legend()
            if total == 'Total':
                graph_area.plot(xfinal + enoffset, (data.variables.ytot * normalization_var) + y0, label='Total', ls='--', lw=2, color='k')  # Plot the simulation of all lines
                graph_area.legend()
            # ------------------------------------------------------------------------------------------------------------------------
            ...
            if load != 'No':
                ...
                y_interp = [0 for i in range(len(exp_x))]
                f_interpolate = interp1d(xfinal + enoffset, (np.array(data.variables.ytot) * normalization_var) + y0, kind='cubic')
                ...
                residues_graph.legend(title="Red. \u03C7\u00B2 = " + "{:.5f}".format(data.variables.chi_sqrd))
            # ------------------------------------------------------------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------------------------------------------------------------------------
                ...
                for cs_index, cs in enumerate(ploted_cs):
                    for index, transition in enumerate(the_dictionary):
                        if the_dictionary[transition]["selected_state"]:
                            ...
                            if not ncs:
                                diag_sim_val = [line + [data.variables.PCS_radMixValues[i].get()] for i, linerad in enumerate(lineradrates_PCS) for line in linerad if line[1] in low_level and line[5] == high_level and float(line[8]) != 0 and rad_PCS[i] == cs]  # Guarda para uma lista as linhas do ficheiro que se referem à trasição transition
                            else:
                                diag_sim_val = [line + [data.variables.NCS_radMixValues[i].get()] for i, linerad in enumerate(lineradrates_NCS) for line in linerad if line[1] in low_level and line[5] == high_level and float(line[8]) != 0 and rad_NCS[i] == cs]

                            if not ncs:
                                sat_sim_val = [line + [data.variables.PCS_radMixValues[rad_PCS.index(sat_PCS[i])].get()] for i, linesat in enumerate(linesatellites_PCS) for line in linesat if low_level in line[1] and high_level in line[5] and float(line[8]) != 0 and sat_PCS[i] == cs]  # Guarda para uma lista as linhas do ficheiro que se referem às satélites de transition
                            else:
                                sat_sim_val = [line + [data.variables.NCS_radMixValues[rad_NCS.index(sat_NCS[i])].get()] for i, linesat in enumerate(linesatellites_NCS) for line in linesat if low_level in line[1] and high_level in line[5] and float(line[8]) != 0 and sat_NCS[i] == cs]  # Guarda para uma lista as linhas do ficheiro que se referem às satélites de transition
                ...
                for cs_index, cs in enumerate(ploted_cs):
                    for index, transition in enumerate(the_aug_dictionary):
                        if the_aug_dictionary[transition]["selected_state"]:
                            ...
                            if not ncs:
                                aug_sim_val = [line + [data.variables.PCS_augMixValues[i].get()] for i, lineaug in enumerate(lineaugrates_PCS) for line in lineaug if low_level in line[1] and high_level in line[5][:2] and auger_level in line[5][2:4] and float(line[8]) != 0 and aug_PCS[i] == cs]  # Guarda para uma lista as linhas do ficheiro que se referem à trasição transition
                            else:
                                aug_sim_val = [line + [data.variables.NCS_augMixValues[i].get()] for i, lineaug in enumerate(lineaugrates_NCS) for line in lineaug if low_level in line[1] and high_level in line[5][:2] and auger_level in line[5][2:4] and float(line[8]) != 0 and aug_NCS[i] == cs]  # Guarda para uma lista as linhas do ficheiro que se referem à trasição transition
            ...
            if load != 'No':  # procedimento para fazer o plot experimental
                ...
                if len(exp_spectrum[0]) >= 3:
                    ...
                    sigma_exp[sigma_exp == 0] = np.min(sigma_exp[sigma_exp != 0])
                if x_mx == 'Auto':
                    max_exp_lim = max(xe)
                else:
                    max_exp_lim = float(x_mx)
            ...
            if sat == 'Diagram':
                for cs_index, cs in enumerate(ploted_cs):
                    for index, key in enumerate(the_dictionary):
                        if the_dictionary[key]["selected_state"]:
                            graph_area.plot(xfinal + enoffset, (np.array(data.variables.yfinal[cs_index * len(the_dictionary) + index]) * normalization_var) + y0, label=cs + ' ' + key, color=str(col2[np.random.randint(0, 7)][0]))  # Plot the simulation of all lines
                            graph_area.legend()
            elif sat == 'Satellites':
                for cs_index, cs in enumerate(ploted_cs):
                    for index, key in enumerate(the_dictionary):
                        if the_dictionary[key]["selected_state"]:
                            for l, m in enumerate(data.variables.yfinals[cs_index * len(the_dictionary) + index]):
                                ...
                                if max(m) != 0:
                                    graph_area.plot(xfinal + enoffset, (np.array(data.variables.yfinals[cs_index * len(the_dictionary) + index][l]) * normalization_var) + y0, label=key + ' - ' + labeldict[label1[l]], color=str(col2[np.random.randint(0, 7)][0]))  # Plot the simulation of all lines
                                    graph_area.legend()
            elif sat == 'Diagram + Satellites':
                for cs_index, cs in enumerate(ploted_cs):
                    for index, key in enumerate(the_dictionary):
                    ...
                    for index, key in enumerate(the_dictionary):
                        if the_dictionary[key]["selected_state"]:
                            for l, m in enumerate(data.variables.yfinals[cs_index * len(the_dictionary) + index]):
                                # Excluir as linhas que nao foram seleccionados nos botoes
                                if max(m) != 0:
                                    graph_area.plot(xfinal + enoffset, (np.array(data.variables.yfinals[cs_index * len(the_dictionary) + index][l]) * normalization_var) + y0, label=cs + ' ' + key + ' - ' + labeldict[label1[l]], color=str(col2[np.random.randint(0, 7)][0]))  # Plot the simulation of all lines
                                    graph_area.legend()
            elif sat == 'Auger':
                for cs_index, cs in enumerate(ploted_cs):
                    for index, key in enumerate(the_aug_dictionary):
                        if the_aug_dictionary[key]["selected_state"]:
                            graph_area.plot(xfinal + enoffset, (np.array(data.variables.yfinal[cs_index * len(the_aug_dictionary) + index]) * normalization_var) + y0, label=cs + ' ' + key, color=str(col2[np.random.randint(0, 7)][0]))  # Plot the simulation of all lines
                            graph_area.legend()
            if total == 'Total':
                graph_area.plot(xfinal + enoffset, (data.variables.ytot * normalization_var) + y0, label='Total', ls='--', lw=2, color='k')  # Plot the simulation of all lines
                graph_area.legend()
        ...
        if len(data.variables.PCS_radMixValues) == 0:
            for cs in radiative_files:
                if '+' in cs:
                    ...
                    CS_mixEntriesRad.append(ttk.Entry(mixer, textvariable=data.variables.PCS_radMixValues[-1], validate='key', validatecommand=check_num_wrapper))
                    slidersRad.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.PCS_radMixValues[-1]))
            ...
            for cs in radiative_files:
                if '+' in cs:
                    CS_mixEntriesRad.append(ttk.Entry(mixer, textvariable=data.variables.PCS_radMixValues[i], validate='key', validatecommand=check_num_wrapper))
                    slidersRad.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.PCS_radMixValues[i]))
                    CS_labelsRad.append(ttk.Label(mixer, text=cs.split('intensity_')[1].split('.out')[0]))
        ...
        if len(data.variables.NCS_radMixValues) == 0:
            for cs in radiative_files:
                if '+' not in cs:
                    ...
                    CS_mixEntriesRad.append(ttk.Entry(mixer, textvariable=data.variables.NCS_radMixValues[-1], validate='key', validatecommand=check_num_wrapper))
                    slidersRad.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.NCS_radMixValues[-1]))
                    CS_labelsRad.append(ttk.Label(mixer, text=cs.split('intensity_')[1].split('.out')[0]))
                    slidersRad[-1].set(0.0)
        else:
            i = 0
            for cs in radiative_files:
                if '+' not in cs:
                    CS_mixEntriesRad.append(ttk.Entry(mixer, textvariable=data.variables.NCS_radMixValues[i], validate='key', validatecommand=check_num_wrapper))
                    slidersRad.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.NCS_radMixValues[i]))
                    CS_labelsRad.append(ttk.Label(mixer, text=cs.split('intensity_')[1].split('.out')[0]))
            ...
            if len(data.variables.PCS_augMixValues) == 0:
                for cs in auger_files:
                    if '+' in cs:
                        ...
                        CS_mixEntriesAug.append(ttk.Entry(mixer, textvariable=data.variables.PCS_augMixValues[-1], validate='key', validatecommand=check_num_wrapper))
                        slidersAug.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.PCS_augMixValues[-1]))
                ...
                for cs in auger_files:
                    if '+' in cs:
                        CS_mixEntriesAug.append(ttk.Entry(mixer, textvariable=data.variables.PCS_augMixValues[i], validate='key', validatecommand=check_num_wrapper))
                        slidersAug.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.PCS_augMixValues[i]))
                        CS_labelsAug.append(ttk.Label(mixer, text=cs.split('augrate_')[1].split('.out')[0]))
            ...
            if len(data.variables.NCS_augMixValues) == 0:
                for cs in auger_files:
                    if '+' not in cs:
                        ...
                        CS_mixEntriesAug.append(ttk.Entry(mixer, textvariable=data.variables.NCS_augMixValues[-1], validate='key', validatecommand=check_num_wrapper))
                        slidersAug.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.NCS_augMixValues[-1]))
                        CS_labelsAug.append(ttk.Label(mixer, text=cs.split('augrate_')[1].split('.out')[0]))
                        slidersAug[-1].set(0.0)
            else:
                i = 0
                for cs in auger_files:
                    if '+' not in cs:
                        CS_mixEntriesAug.append(ttk.Entry(mixer, textvariable=data.variables.NCS_augMixValues[i], validate='key', validatecommand=check_num_wrapper))
                        slidersAug.append(ttk.Scale(mixer, orient=VERTICAL, length=200, from_=100.0, to=0.0, variable=data.variables.NCS_augMixValues[i]))
                        CS_labelsAug.append(ttk.Label(mixer, text=cs.split('augrate_')[1].split('.out')[0]))
            ...
            for cs in Ion_Population_Functions:
                if len(data.variables.PCS_radMixValues) > 0:
                    ...
                    for cs_file in radiative_files:
                        if cs in cs_file:
                            try:
                                data.variables.PCS_radMixValues[i].set(str(Ion_Population_Functions[cs](float(temperature.get()))))
                            except:
                ...
                if len(data.variables.NCS_radMixValues) > 0:
                    ...
                    for cs_file in radiative_files:
                        if cs in cs_file:
                            try:
                                data.variables.NCS_radMixValues[i].set(str(Ion_Population_Functions[cs](float(temperature.get()))))
                            except:
                ...
                for cs in Ion_Population_Functions:
                    if len(data.variables.PCS_augMixValues) > 0:
                        ...
                        for cs_file in auger_files:
                            if cs in cs_file:
                                try:
                                    data.variables.PCS_augMixValues[i].set(str(Ion_Population_Functions[cs](float(temperature.get()))))
                                except:
                    ...
                    if len(data.variables.NCS_augMixValues) > 0:
                        ...
                        for cs_file in auger_files:
                            if cs in cs_file:
                                try:
                                    data.variables.NCS_augMixValues[i].set(str(Ion_Population_Functions[cs](float(temperature.get()))))
                                except:
